...
2. Under **VirtualBox platform packages**, click **"macOS / Apple Silicon hosts"**
   - This downloads the ARM-compatible `.dmg` installer
3. Also download the **VirtualBox Extension Pack** from the same page (match the version number exactly)
...
4. Download: `kali-linux-2024.x-installer-arm64.iso`
...
```bash
shasum -a 256 ~/Downloads/kali-linux-2024.x-installer-arm64.iso
```
...
| **ISO Image** | Browse → select your `kali-linux-arm64.iso` |
| **Type** | Linux |
...
*MacBook Pro M1 · Kali Linux ARM64 · VirtualBox 7.1+*
